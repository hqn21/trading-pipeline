...
class Trainer:
    ...
    def _get_loss_fn(self, loss_name):
        ...
        if loss_name not in losses:
            raise ValueError(
                f"Loss '{loss_name}' not supported. Available: {list(losses.keys())}"
            )
        return losses[loss_name]

    def _prepare_batch(self, Xs, Xs_broker, Xs_general, Ys):
        ...
        return Xs, Xs_broker, Xs_general, Ys

    def _masked_loss(self, preds, Ys, Xs, Xs_broker):
        nan_mask_Xs = torch.isnan(Xs).any(dim=(2, 3))
        nan_mask_Xs_broker = torch.isnan(Xs_broker).any(dim=(2, 3))
        nan_mask_Ys = torch.isnan(Ys)
        valid_mask = (~(nan_mask_Xs | nan_mask_Xs_broker | nan_mask_Ys)).float()
    ...
    def _train_one_epoch(self, epoch, loader):
        ...
        for batch_idx, (Xs, Xs_broker, Xs_general, Ys, _) in progress_bar:
            Xs, Xs_broker, Xs_general, Ys = self._prepare_batch(
                Xs, Xs_broker, Xs_general, Ys
            )
            self.optimizer.zero_grad()
            preds = self.model(Xs, Xs_broker, Xs_general)
            loss = self._masked_loss(preds, Ys, Xs, Xs_broker)
            loss.backward()
            self.optimizer.step()
    ...
    def evaluate(self, loader):
        ...
        total_loss = 0.0
        with torch.no_grad():
            for Xs, Xs_broker, Xs_general, Ys, _ in loader:
                Xs, Xs_broker, Xs_general, Ys = self._prepare_batch(
                    Xs, Xs_broker, Xs_general, Ys
                )
                preds = self.model(Xs, Xs_broker, Xs_general)
                total_loss += self._masked_loss(
                    preds, Ys, Xs, Xs_broker
                ).item()
    ...
    def _evaluate_and_save(
        ...
        self.model.eval()
        with torch.no_grad():
            for Xs, Xs_broker, Xs_general, Ys, indices in loader:
                Xs, Xs_broker, Xs_general, Ys = self._prepare_batch(
                    Xs, Xs_broker, Xs_general, Ys
                )
                preds = self.model(Xs, Xs_broker, Xs_general)
                ...
                mask = ~(
                    ...
                    torch.isnan(Xs_broker).any(dim=(2,3)) |
                    torch.isnan(Ys)
                )
