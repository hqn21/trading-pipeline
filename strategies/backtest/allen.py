...
class Strategy():
    ...
    def init(self, buy_signal: pd.DataFrame, sell_signal: pd.DataFrame, targets: list, max_positions: int, is_benchmark: bool):
        """
        ...
        for symbol in self.symbols:
            ...
            self.sell_signal[symbol] = sell_signal[symbol].values if symbol in sell_signal.columns else [0] * len(buy_signal)
    ...
    def next(self, i: int, record: Dict[Hashable, Any]):
        ...
        # model: buy when buy signal occurs.  sell when sell signal occurs
            ...
            for position in self.open_positions[:]:
                ...
                #elif (record[(position.symbol,'Open')] < position.open_price * 0.9):
                #    self.close(position=position, price=record[(position.symbol,'Open')], exit_reason="Stop profit")  
            
            for symbol in self.symbols:
                if (len(self.open_positions) == self.max_positions): # No open positions now
                    break
    ...
    def close(self, price: float, symbol: Optional[str] = None, position: Optional[Position] = None, exit_reason: Optional[str] = None):
        """
    ...
    def __eval(self, *args, **kwargs):
        ...
        for i, record in enumerate(self.records):
            ...
            self.invest_ratio.append(self.assets_value / (self.cash + self.assets_value))
