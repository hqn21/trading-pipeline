import methods.allen_signal
import methods.gino_signal
import pandas as pd
import yaml
...
def generate_buy_signal(pred_df, method: str, val_df=None):
    if (method == "allen"):
        ...
        return methods.allen_signal.get_buy_signals(pred_df, val_df, cfg)
...
def generate_sell_signal(pred_df:pd.DataFrame, method: str, val_df=None, buy_signal:pd.DataFrame=None):
    if (method == "allen"):
        ...
        cfg["buy_percentile"] = 0.75
        cfg["sell_percentile"] = 0.25
        return methods.allen_signal.get_sell_signals(pred_df, val_df, cfg)
